...
# Needed to correctly format date and time values
    ...
    if request.method == 'GET':
        ...
        if departure_country:
            queryset = queryset.filter(
                departure_country__country_name=departure_country)
        if arrival_country:
            queryset = queryset.filter(
                arrival_country__country_name=arrival_country)
        if max_price:
        ...
        for flight in queryset:
            departure_country_name = Country.objects.get(
                id=flight.departure_country_id).country_name
            arrival_country_name = Country.objects.get(
                id=flight.arrival_country_id).country_name
...
def book(request):
    if request.method == 'POST' and request.content_type == 'application/json':
        ...
        lead_passenger_contact_email = request_data.get(
            'lead_passenger_contact_email')
        lead_passenger_contact_number = request_data.get(
            'lead_passenger_contact_number')

        all_instances_of_flight_id = Flight_Instance.objects.filter(
            id=flight_id).value("flight_ticket_cost")
        total_booking_cost = all_instances_of_flight_id["flight_ticket_cost"] * len(
            all_instances_of_flight_id)
        ...
        url = 'http://lanre.pythonanywhere.com/pay'
        data = {
            'sender_cardholder_name': cardholder_name,
            'sender_card_number_hash': card_number,
            'sender_cvc_hash': cvc,
            'sender_expiry_date': expiry_date,
            # Get details from payment database
            'recipient_cardholder_name': payment_details['keeganAirline'],
            # Get details from payment database
            'recipient_sortcode': payment_details['232323'],
            # Get details from payment database
            'recipient_account_number': payment_details['365555721'],
...
def delete(request):
    ...
    if request.method == 'DELETE':
        ...
        booking_instance_to_be_deleted = Booking_Instance.objects.get(
            id=booking_id)
